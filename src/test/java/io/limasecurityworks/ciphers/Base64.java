...
public class Base64 {
    ...
    public void runBase64() {
        ...
        if (this.help != null) {
            ...
            if (this.function != null) {
                System.out.println("You have entered a command with a function to either encrypt or decrypt. There are no keys for the Base64 Cipher. You will next be prompted to input your text to be encrypted or decrypted after this command - help.");
            }
            else {
                System.out.println("Base64 Cipher will ask you if you would like to encrypt or decrypt and then it will ask for input. There is not a key to be input for this cipher. You will first be prompted to specify if you would like to encrypt or decrypt after entering this command - help.");
            }
        }

        else if (this.about != null) {
            System.out.println("Base64 is a group of similar binary-to-text encoding schemes that represent binary data in an ASCII string format by translating it into a radix-64 representation. The term Base64 originates from a specific MIME content transfer encoding. Each Base64 digit represents exactly 6 bits of data. Three 8-bit bytes (i.e., a total of 24 bits) can therefore be represented by four 6-bit Base64 digits." + " " + "(\"Autokey cipher,\" n.d.)");
        }
    ...
    /*
    * Steps to decryption of a base64 string.
    * 1. Turn the string into binary using hashmap. Each letter will produce 6 digits.
    * 2. Convert the binary string back to letters using 8 digits per character.
    */
